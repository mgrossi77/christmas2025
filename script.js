...
// Get personalized name from URL parameter
var urlParams = new URLSearchParams(window.location.search);
var recipientName = urlParams.get('name');

// Update the message with the personalized greeting
if (recipientName) {
    document.querySelector('#endMessage .endMessage').innerHTML = 
        'Merry Christmas, ' + recipientName + '! 🎄';
}

function r(a){gsap.killTweensOf(a,{opacity:!0});gsap.fromTo(a,{opacity:1},{duration:.07,opacity:Math.random(),repeat:-1})}function t(a){e&&(a=l[d],gsap.set(a,{x:gsap.getProperty(".pContainer","x"),y:gsap.getProperty(".pContainer","y"),scale:m()}),gsap.timeline().to(a,{duration:gsap.utils.random(.61,6),physics2D:{velocity:gsap.utils.random(-23,23),angle:gsap.utils.random(-180,180),gravity:gsap.utils.random(-6,50)},scale:0,rotation:gsap.utils.random(-123,360),ease:"power1",onStart:r,onStartParams:[a],
